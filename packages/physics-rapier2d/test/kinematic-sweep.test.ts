...
describe("Rapier 2D kinematic sweep", () => {
  it("stops a circle origin outside the blocker and preserves the contact witness", async () => {
    const backend = await initRapier2dPhysicsBackend({ id: "rapier2d.kinematic-sweep" });
    const scene = backend.createScene({ gravity: { x: 0, y: 0 }, fixedDeltaMs: 100 });
...
function sceneQueries(
  scene: ReturnType<Awaited<ReturnType<typeof initRapier2dPhysicsBackend>>["createScene"]>
