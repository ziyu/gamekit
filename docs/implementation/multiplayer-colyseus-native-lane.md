...
- Demo session 现在显式选择 `gamekit-envelope` 或 `colyseus-schema`，两条 lane 复用同一 app-local snapshot、prediction 和 presentation pipeline，不双写 authoritative gameplay state。
- `multiplayer-core` authority loop 支持可选 provider snapshot publisher，同时保留 tick/capture/diagnostics/error contract。
- `@gamekit/multiplayer-colyseus` 提供真实 Schema room state、host-only publisher、typed subscription/bridge、provider state version 排序、重复 callback 去重、source/session/size/stale gate 和 diagnostics。
...
- `@gamekit/multiplayer-core`: 37 tests passed。
- `@gamekit/multiplayer-colyseus`: 9 tests passed，包含真实 Schema room、authority writer 和非法 writer/version 拒绝。
