...
规划并落地统一 Multiplayer package 体系，让 GameKit 可以通过成熟多人 backend 支持本地测试、Colyseus Room 和未来托管/平台 backend，同时保持 core facade、App Host service、GameModule bridge 和 backend adapter 的边界清晰。
...
corepack pnpm --filter @gamekit/multiplayer-core lint
corepack pnpm --filter @gamekit/multiplayer-memory lint
```

2026-07-01 已继续落地 App Host 组合层：

- `@gamekit/app-host` 增加 optional standard multiplayer service，暴露 `services.multiplayer`，Host dispose 默认释放 MultiplayerRuntime。
...
```bash
corepack pnpm --filter @gamekit/app-host test
corepack pnpm --filter @gamekit/devtools test
corepack pnpm --filter @gamekit/devtools build
corepack pnpm --filter @gamekit/multiplayer-core build
corepack pnpm --filter @gamekit/app-host build
corepack pnpm --filter @gamekit/app-host lint
corepack pnpm --filter @gamekit/devtools lint
corepack pnpm --filter @gamekit/multiplayer-core lint
corepack pnpm test
corepack pnpm build
corepack pnpm lint
...
目标：建立不依赖任何外部网络 SDK 的 GameKit 侧窄 facade，并避免它继续膨胀成自研多人 runtime。
...
- `@gamekit/multiplayer-core` 不依赖具体 backend SDK、DOM/Node socket、Colyseus、Nakama 或 provider 类型。
...
- `corepack pnpm test --filter @gamekit/multiplayer-core` 通过。

### Wave 2: Memory Backend And Conformance

目标：用 in-process test backend 固定 GameKit facade/bridge 行为契约。
...
2. 实现 Colyseus client adapter，把 Room join/leave/send/onMessage/onStateChange/onLeave 映射到 GameKit facade。
...
- Colyseus server harness 是否作为 `@gamekit/multiplayer-colyseus/server` 子入口，还是放在 app-specific server project。
- Command schema 校验优先使用 DataTypeDefinition、轻量自定义 validator，还是交给 app 注入第三方 schema adapter。
- Rollback 是否长期保留在 multiplayer-core 的最小 hook 内，还是单独形成 `@gamekit/multiplayer-rollback` toolkit。
