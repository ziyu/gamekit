...
Outpost Siege 的 authority checkpoint 必须恢复 Physics、GAS 和 TCA 后继续确定性 tick。`@gamekit/save` 已定义通用 contributor、section、validation 和 entity map 协议，但不能反向依赖所有 gameplay domain；各 app 也不应通过闭包读取 domain runtime 私有状态并重复实现序列化。
...
- `@gamekit/physics-core` 提供 `createPhysicsSaveContributor()`。
- `@gamekit/gas` 提供 `createGasSaveContributor()`。
- `@gamekit/tca` 提供 `createTcaSaveContributor()`。

领域包单向依赖 `@gamekit/save` 的稳定 contributor/section/context 类型。`@gamekit/save` 不依赖 Physics、GAS 或 TCA，也不理解 section 内部结构。
...
- Physics、GAS、TCA 发布包新增对 `@gamekit/save` 协议包的依赖。
...
### Make `@gamekit/save` import every gameplay domain
