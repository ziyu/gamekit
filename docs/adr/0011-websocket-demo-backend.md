...
`@gamekit/multiplayer-memory` 可以固定 backend adapter 契约、消息顺序和 headless 测试夹具，但它不经过真实 transport，也不暴露 socket close、跨进程消息编码、服务端 relay、连接失败和重连降级这类问题。

Multiplayer demo 的目标不是只证明 core runtime 能在同一进程里调用，而是证明 GameKit 的多人 package 体系可以通过一个真实 backend 跑通：App Host 管连接服务，GameModule bridge 管 tick 边界和 authority，DevTools/diagnostics 能解释网络链路，gameplay 不依赖 backend 私有类型。

托管 room service、平台联机 SDK 或完整 headless authoritative server 都更接近生产形态，但作为首个真实 backend 会同时引入账号、部署、鉴权、云端运维或 provider-specific room model，容易把 demo 验证范围扩大到 GameKit 当前多人包边界之外。

## Decision

首个跑通 demo 的真实 backend 选择 `@gamekit/multiplayer-websocket`。
...
- `@gamekit/multiplayer-core` 不依赖 DOM WebSocket、Node socket、server framework 或 backend-specific 类型。
- `@gamekit/multiplayer-memory` 保留为 conformance、确定性测试和本地 loopback 替身，但不能作为 demo 最终跑通的唯一 backend。
...
- WebSocket 是通用、低门槛、可本地启动的真实 backend，不需要账号或云服务才能验证 GameKit 包边界。
...
- 需要新增 `@gamekit/multiplayer-websocket` package、server-side 测试夹具和本地 demo 启动方式。
