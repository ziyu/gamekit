...
`@gamekit/multiplayer-colyseus/server` 提供一个小型、版本化的 `GameKitColyseusNativeState` Schema carrier。它只保存 session/source identity、gameplay tick、app schema version、timestamp、编码后的 app-owned state、state bytes 和单调 `updateCount`。

- 每个 room 只声明一个 authoritative path。选择 Schema 时，GameKit envelope 不再发布同一 gameplay snapshot。
...
- GameKit 能真实使用 Colyseus Schema patch lifecycle，同时保持 gameplay snapshot 和 presentation pipeline provider-neutral。
