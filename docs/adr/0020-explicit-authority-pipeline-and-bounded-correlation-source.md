...
- TCA、GAS、Physics trace store factory 提供可选 `onEntry` hook。Domain package 仍不依赖 DevTools；未配置 hook 时不增加 observer 工作。
- `@gamekit/app-host` 提供 `createGameplayDevToolsCorrelation(...)` 组合 helper，创建三套 domain trace store 并把稳定、小型 trace 映射到同一个 DevTools source。
...
Performance gate 使用 50,000 条多模块 trace、512 条 runtime trace 上限和 64 条 correlation summary 上限，分别约束增量写入成本、snapshot 成本和 retained size。
...
- `onEntry` hook 中的自定义逻辑必须保持小且不执行 gameplay mutation。
